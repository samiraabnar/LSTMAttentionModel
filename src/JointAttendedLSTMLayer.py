...
class JointAttendedLSTMLayer(object):
    ...
    def __init__(self,random_state,input,input_dim,output_dim,outer_output_dim,bptt_truncate=-1,layer_id="_0"):
        ...
        def forward_step(x_t, prev_state, prev_content, prev_state_2, prev_content_2):
            ...
            attention = T.nnet.sigmoid(T.dot(self.Att_w,s2)+self.Att_bias)
            return [o,attention,s1,c,s2,c2,input_gate, forget_gate, output_gate]

        [self.output,self.attention,self.hidden_state,self.memory_content,self.hidden_state_2,self.memory_content_2,self.input_gate, self.forget_gate,
         self.output_gate] , updates = theano.scan(
            forward_step,
            sequences=[self.input],
            truncate_gradient=self.bptt_truncate,
            outputs_info=[None,None,dict(initial=T.zeros(self.output_dim,dtype=theano.config.floatX)),
    ...
    def initialize_params(self):
        ...
        Att_w = orthogonal_weight(1, self.outer_output_dim, scale=0.01)
        """np.asarray(
            self.random_state.normal(-np.sqrt(6.0 / (self.output_dim + self.output_dim)),
                                     np.sqrt(6.0 / (self.output_dim + self.output_dim)),
                                     (1, self.output_dim))
            , dtype=theano.config.floatX)
        """
        Att_bias = np.zeros(1, dtype=theano.config.floatX)

        self.Att_w = theano.shared(value=O_w, name="Att_w" + self.layer_id, borrow="True")
        self.Att_bias = theano.shared(value=O_bias, name="Att_bias" + self.layer_id, borrow="True")
        ...
        self.output_params = [self.O_w,self.O_bias,self.Att_w,self.Att_bias]